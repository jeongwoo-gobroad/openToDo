...
컴파일 방법: 
	...
	*** 디렉토리 내의 파일에 대해 chmod 755 * 를 수행하는 것을 추천함.
...
실행 방법: 
	...
	3. 구동 필수 파일
		public.dsv
			개인 저장 파일이 아닌, 공휴일 데이터베이스를 저장하고 있는 파일입니다.
			해당 파일이 반드시 실행 경로에 존재해야 하며, 존재하지 않을시 프로그램을 강제 종료합니다.
	4. 저장 파일
		바이너리 파일
			todos.sv: 존재 할 경우 실행과 동시에 자동으로 불러오고, 존재하지 않는다면 생성합니다.
		Human-readable 파일
			todos.txt: 저장 액션 수행시 생성되는 파일입니다. 
